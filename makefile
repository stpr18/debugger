PROGRAM_NAME := Debugger

ifeq ($(OS),Windows_NT)
	CXX := D:\system\TDM-GCC-64\bin\g++
	MKDIR = if NOT EXIST $(subst /,\,$(1)) md $(subst /,\,$(1))
	BOOST_PATH := "$(BOOST_INCLUDE)"
	BOOST_LIB_PATH := "$(BOOST_LIB_X64)"
else
	CXX := g++
	MKDIR = mkdir -p $(1)
...
CXXFLAGS := -Wall -Wextra -lm -O3 -march=native -fomit-frame-pointer -fopenmp -std=c++11 -Wno-missing-field-initializers -I$(BOOST_PATH) -L$(BOOST_LIB_PATH) -municode
LIBS := 
